...
public interface BookCacheDao {
    ...
    List<com.wkq.database.bean.BookCache> getBookCaches(String bookName, String siteName);

    @Query("select * from bookCache where bookName = :bookName and siteName = :siteName and `index` = :index")
    com.wkq.database.bean.BookCache getBookCache(String bookName, String siteName, int index);
    ...
    @Query("select * from bookCache where bookName = :bookName and siteName = :siteName order by `index` asc")
    DataSource.Factory<Integer, com.wkq.database.bean.BookCache> getCachesFactory(String bookName, String siteName);
